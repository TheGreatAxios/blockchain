...
import 'package:blockchain/libraries/crypto.dart' show Formatter;
import 'package:blockchain/utils/length_tracking_byte_sink.dart';
import 'package:blockchain/utils/typed_data.dart' show TypedData;
...
void _encodeString(Uint8List string, LengthTrackingByteSink builder) {
  ...
  final encodedLength = Formatter.unsignedIntToBytes(BigInt.from(length));
...
void _encodeInt(BigInt val, LengthTrackingByteSink builder) {
  if (val == BigInt.zero) {
    ...
    _encodeString(Formatter.unsignedIntToBytes(val), builder);
  }
}
...
void _encodeToBuffer(dynamic value, LengthTrackingByteSink builder) {
  if (value is Uint8List) {
    ...
    _encodeString(TypedData.uint8ListFromList(utf8.encode(value)), builder);
  } else {
