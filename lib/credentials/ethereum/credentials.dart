...
import 'package:blockchain/credentials/ethereum/address.dart';
import 'package:blockchain/crypto/formatter.dart';
import 'package:blockchain/crypto/keccak.dart';
import 'package:blockchain/crypto/msg_signature.dart';
import 'package:blockchain/crypto/secp256k1.dart' as secp256k1;
import 'package:blockchain/utils/typed_data.dart';
import 'package:collection/collection.dart';
import 'package:pointycastle/ecc/api.dart' show ECPoint;

/// Anything that can sign payloads with a private key.
abstract class EthereumCredentials {
  ...
  Future<Uint8List> sign(Uint8List payload,
    ...
    final r = TypedData.padUint8ListTo32(Formatter.unsignedIntToBytes(signature.r));
    final s = TypedData.padUint8ListTo32(Formatter.unsignedIntToBytes(signature.s));
    final v = Formatter.unsignedIntToBytes(BigInt.from(signature.v));

    // https://github.com/ethereumjs/ethereumjs-util/blob/8ffe697fafb33cefc7b7ec01c11e3a7da787fe0e/src/signature.ts#L63
    return TypedData.uint8ListFromList(r + s + v);
  }
  ...
  Future<Uint8List> signPersonalMessage(Uint8List payload, {int? chainId}) {
    ...
    final concat = TypedData.uint8ListFromList(prefixBytes + payload);
...
abstract class EthereumCredentialsWithKnownAddress extends EthereumCredentials {
...
abstract class CustomTransactionSender extends EthereumCredentials {
  Future<String> sendTransaction(TransactionRequest transaction);
}

/// Credentials that can sign payloads with an Ethereum private key.
class EthPrivateKey extends EthereumCredentialsWithKnownAddress {
  ...
  EthPrivateKey(this.privateKey)
      : privateKeyInt = Formatter.bytesToUnsignedInt(privateKey);

  /// Parses a private key from a hexadecimal representation.
  EthPrivateKey.fromHex(String hex) : this(Formatter.hexToBytes(hex));

  /// Creates a private key from the underlying number.
  EthPrivateKey.fromInt(this.privateKeyInt)
      : privateKey = Formatter.unsignedIntToBytes(privateKeyInt);
  ...
  factory EthPrivateKey.createRandom(Random random) {
    ...
    return EthPrivateKey(Formatter.intToBytes(key));
  }
  ...
  EthereumAddress get address {
    return _cachedAddress ??=
        EthereumAddress(secp256k1.publicKeyToAddress(secp256k1.privateKeyToPublic(privateKeyInt)));
  }

  /// Get the encoded public key in an (uncompressed) byte representation.
  Uint8List get encodedPublicKey => secp256k1.privateKeyToPublic(privateKeyInt);

  /// The public key corresponding to this private key.
  ECPoint get publicKey => (secp256k1.params.G * privateKeyInt)!;
