...
import 'package:blockchain/crypto/secp256k1.dart';
...
class EthereumAddress {
  ...
  factory EthereumAddress.fromHex(String hex, {bool enforceEip55 = false}) {
    ...
    if (!enforceEip55 &&
      ...
      return EthereumAddress(Formatter.hexToBytes(hex));
    }

    // Validates as of EIP 55, https://ethereum.stackexchange.com/a/1379
    final address = Formatter.remove0x(hex);
    final hash = Formatter.bytesToHex(keccakAscii(address.toLowerCase()));
    for (var i = 0; i < 40; i++) {
  ...
  String get hex =>
      Formatter.bytesToHex(addressBytes, include0x: true, forcePadLength: 40);

  /// A hexadecimal representation of this address, padded to a length of 40
  /// characters or 20 bytes, but not prefixed with "0x".
  String get hexNo0x =>
      Formatter.bytesToHex(addressBytes, include0x: false, forcePadLength: 40);
  ...
  String get hexEip55 {
    ...
    final hash = Formatter.bytesToHex(keccakAscii(hexNo0x));
