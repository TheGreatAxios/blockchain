...
import 'package:blockchain/credentials/ethereum/credentials.dart';
...
class _PBDKDF2KeyDerivator extends _KeyDerivator {
  ...
  Map<String, dynamic> encode() {
    return {
      ...
      'dklen': dklen,
      'prf': 'hmac-sha256',
      'salt': Formatter.bytesToHex(salt)
    };
  }
...
class Wallet {
  ...
  String toJson() {
    ...
    final map = {
      'crypto': {
        'cipher': 'aes-128-ctr',
        'cipherparams': {'iv': Formatter.bytesToHex(_iv)},
        'ciphertext': Formatter.bytesToHex(ciphertextBytes),
        'kdf': _derivator.name,
  ...
  factory Wallet.createNew(
    ...
    final RandomBridge dartRandom = RandomBridge(random);

    final Uint8List salt = dartRandom.nextBytes(32);
    final _ScryptKeyDerivator derivator = _ScryptKeyDerivator(32, scryptN, 8, p, salt);

    final Uint8List uuid = UUID.generateUuidV4();
  ...
  factory Wallet.fromJson(String encoded, String password) {
    ...
    */
    ...
    switch (kdf) {
      case 'pbkdf2':
        ...
        derivator = _PBDKDF2KeyDerivator(
            derParams['c'] as int,
            Uint8List.fromList(Formatter.hexToBytes(derParams['salt'] as String)),
    ...
    final encryptedPrivateKey = Formatter.hexToBytes(crypto['ciphertext'] as String);
    ...
    final iv =
        Uint8List.fromList(Formatter.hexToBytes(crypto['cipherparams']['iv'] as String));
  ...
  static String _generateMac(List<int> dk, List<int> ciphertext) {
    ...
    return Formatter.bytesToHex(keccak256(TypedData.uint8ListFromList(macBody)));
  }
