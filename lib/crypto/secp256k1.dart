...
Uint8List privateKeyBytesToPublic(Uint8List privateKey) {
  return privateKeyToPublic(Formatter.bytesToUnsignedInt(privateKey));
}
...
MsgSignature sign(Uint8List messageHash, Uint8List privateKey) {
  ...
  final key = ECPrivateKey(Formatter.bytesToUnsignedInt(privateKey), params);
  ...
  final publicKey = Formatter.bytesToUnsignedInt(privateKeyBytesToPublic(privateKey));
...
Uint8List ecRecover(Uint8List messageHash, MsgSignature signatureData) {
  final r = TypedData.padUint8ListTo32(Formatter.unsignedIntToBytes(signatureData.r));
  final s = TypedData.padUint8ListTo32(Formatter.unsignedIntToBytes(signatureData.s));
  assert(r.length == 32);
  assert(s.length == 32);
  ...
  return Formatter.unsignedIntToBytes(pubKey);
}
...
bool isValidSignature(
  ...
  return Formatter.bytesToHex(publicKey) == Formatter.bytesToHex(recoveredPublicKey);
}
...
BigInt? _recoverFromSignature(
  ...
  final e = Formatter.bytesToUnsignedInt(msg);
  ...
  return Formatter.bytesToUnsignedInt(bytes.sublist(1));
}
...
ECPoint _decompressKey(BigInt xBN, bool yBit, ECCurve c) {
  List<int> x9IntegerToBytes(BigInt s, int qLength) {
    ...
    final bytes = Formatter.intToBytes(s);
