...
import 'package:blockchain/utils/typed_data.dart' as typed;
...
class UintType extends _IntTypeBase {
  ...
  void encode(BigInt data, LengthTrackingByteSink buffer) {
    ...
    final bytes = Formatter.unsignedIntToBytes(data);
  ...
  BigInt _decode32Bytes(Uint8List data) {
    ...
    // them.
    return Formatter.bytesToUnsignedInt(data);
  }
...
class IntType extends _IntTypeBase {
  ...
  void encode(BigInt data, LengthTrackingByteSink buffer) {
    ...
    if (negative) {
      // twos complement
      bytesData = Formatter.unsignedIntToBytes((BigInt.one << length) + data);
    } else {
      bytesData = Formatter.unsignedIntToBytes(data);
    }
  ...
  @override
  BigInt _decode32Bytes(Uint8List data) {
    return Formatter.bytesToInt(data);
  }
